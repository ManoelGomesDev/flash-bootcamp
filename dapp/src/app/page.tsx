...
import { Address, createWalletClient, custom, getContract } from "viem";
...
const loadTasks = async () => {
  ...
  try {
      ...
      try {
        ...
        if (task.owner.toLowerCase() === account.toLowerCase()) {
          loadedTasks.push({
            ...
            dueDate: new Date(Number(task.dueDate) * 1000).toISOString().slice(0, 16),
    ...
    setTaskCount(id)
    console.log(`Carregadas ${loadedTasks.length} tarefas do usuário`)
  } catch (error) {
  ...
  return (
    <div className="flex flex-col min-h-screen max-w-7xl mx-auto pt-10">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4 mt-10">
        ...
        <StatusCard title="Total de tarefas concluídas" value={tasks.filter(task => task.isCompleted).length} />
        <StatusCard title="Total de tarefas pendentes" value={tasks.filter(task => !task.isCompleted).length} />
